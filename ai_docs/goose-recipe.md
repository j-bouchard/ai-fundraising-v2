...
After creating recipe files, you can use [`goose` CLI commands](https://block.github.io/goose/docs/guides/goose-cli-commands) to run or validate the files and to manage recipe sharing.

### CLI and Desktop Formats

Goose recipes use two formats:

- **CLI Format**: Recipe fields (like `title`, `description`, `instructions`) are at the root level of the YAML/JSON file. This format is used when recipes are created via the CLI `/recipe` command and [Recipe Generator](https://block.github.io/goose/recipe-generator) YAML option.
...
The CLI automatically detects and handles both formats when running `goose run --recipe <file>` and `goose recipe` commands. The Desktop can [import](https://block.github.io/goose/docs/guides/recipes/storing-recipes#importing-recipes) and use YAML recipes (or deeplinks) in either CLI or Desktop format.
...
| `requirement` | String | One of: "required", "optional", or "user\_prompt" |
...
The `sub_recipes` field specifies the [subrecipes](https://block.github.io/goose/docs/guides/recipes/subrecipes) that the main recipe calls to perform specific tasks. Each subrecipe in the array has the following structure:
...
| `sequential_when_repeated` | Boolean | (Optional) Forces sequential execution of multiple subrecipe instances. See [Running Subrecipes In Parallel](https://block.github.io/goose/docs/tutorials/subrecipes-in-parallel) for details |
...
| `on_failure_timeout_seconds` | Number | (Optional) Timeout for on\_failure commands (default: 600 seconds) |
...
3. **Retry Decision**: If any success check fails and retry attempts remain:
	- Execute the on\_failure command (if configured)
...
- `GOOSE_RECIPE_ON_FAILURE_TIMEOUT_SECONDS`: Global timeout for on\_failure commands
...
This **enables automation** by returning consistent, parseable results for scripts and workflows. Recipes can produce structured output when run from either the Goose CLI or Goose Desktop. See [use cases and ideas for automation workflows](https://block.github.io/goose/docs/guides/recipes/session-recipes#structured-output-for-automation).
...
- **Timeout errors**: Success checks or on\_failure commands that exceed their timeout limits
...
Check out the [Goose Recipes](https://block.github.io/goose/docs/guides/recipes) guide for more docs, tools, and resources to help you master Goose recipes.